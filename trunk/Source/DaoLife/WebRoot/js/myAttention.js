function friendsHot(id,index,name,content,picurl,baseinfo,otherinfo,attention,people){
	...
	this.picurl = picurl || 'images/myhome_30.gif';
	this.baseinfo = baseinfo || '';
	this.otherinfo = otherinfo || '';
	this.attention = attention || false;
...
function doReload(fn){
	...
	var func = function(rs){
		...
		for(var i = 0, l = rs.items.length; i < l; ++i){
			...
			myBox.articleBox.add(new friendsHot(rs.items[i].userId,((rs.currentPage - 1) * rs.pageSize) + (i + 1),rs.items[i].userNickName,rs.items[i].userInfo,'images/myhome_30.gif',baseinfo,otherinfo,rs.items[i].followFlag,rs.items[i].fansNum));
		}
		if(fn){
			fn();
		}
	}
	DaolifeAjax.getMyFollowFriend(myBox.articleBox.currentPage,func);
}
$(function($){
	doReload(function(){
		myBox.articleBox.load();
